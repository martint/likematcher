...
record NFA(State start, State accept, List<State> states, Map<Integer, List<Transition>> transitions)
{
    ...
    public DFA toDFA()
    {
        ...
        Set<State> initial = transitiveClosure(Set.of(this.start));
        ...
        while (!queue.isEmpty()) {
            ...
            for (int byteValue = 0; byteValue < 256; byteValue++) {
                ...
                DFA.State to = failed;
                if (!next.isEmpty()) {
                    Set<State> closure = transitiveClosure(next);
                    to = activeStates.computeIfAbsent(closure, nfaStates -> builder.addState(makeLabel(nfaStates), closure.contains(accept)));
                    queue.add(closure);
                }
    ...
    {
        List<Transition> result = transitions.get(state.id());
        if (result == null) {
            return List.of();
        }
    ...
    public static record State(int id)
    {
        @Override
    ...
    static record Epsilon()
            implements Condition {}

    static record Value(byte value)
            implements Condition {}

    static record Prefix(int prefix, int bits)
            implements Condition {}
}
